        ...
        <nav class="mb-6">
          <ol class="flex items-center gap-2 text-sm text-stone-500">
            ...
            <li class="flex items-center gap-2">
              <span>/</span>
              <span class="text-stone-900">Wunschliste</span>
            </li>
          </ol>
        </nav>
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-stone-500 mt-2">
              {{ likeCount }} {{ likeCount === 1 ? 'Artikel' : 'Artikel' }} gespeichert
              <span v-if="isSyncing" class="ml-2 text-amber-600">
    ...
    <section class="py-12">
      <div class="max-w-7xl mx-auto px-6">
        ...
        <div v-if="likeCount === 0" class="text-center py-20">
          ...
          <h2 class="font-serif text-2xl text-stone-900 mb-3">Ihre Wunschliste ist leer</h2>
          <p class="text-stone-500 mb-8 max-w-md mx-auto">
            Fügen Sie Artikel hinzu, indem Sie auf das Herz-Symbol bei einem Produkt klicken.
          </p>
          <NuxtLink 
            to="/furniture/all"
        ...
        <div v-else>
          ...
          <div class="flex items-center justify-between mb-8 pb-6 border-b border-stone-100">
            <p class="text-stone-600">
              {{ likeCount }} gespeicherte Artikel
            </p>
            <button 
              @click="confirmClearAll"
              :disabled="isClearing"
              class="text-sm text-stone-500 hover:text-rose-600 transition-colors disabled:opacity-50"
            >
              <span v-if="isClearing">Wird gelöscht...</span>
              <span v-else>Alle entfernen</span>
            </button>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            ...
            >
              ...
              <button 
                ...
                :class="{ 'opacity-50': removingIds.includes(product.key) }"
                :disabled="removingIds.includes(product.key)"
                @click.stop.prevent="handleRemove(product.key)"
                aria-label="Von Wunschliste entfernen"
              >
                <svg 
                  v-if="removingIds.includes(product.key)"
                  class="w-5 h-5 text-rose-500 animate-spin"
                  fill="none" 
                  viewBox="0 0 24 24"
                >
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
              ...
              <p class="text-xs text-stone-400 mt-2">
                Hinzugefügt {{ formatDate(product.addedAt) }}
              </p>
            </div>
          </div>
          ...
          <div class="text-center mt-16 pt-12 border-t border-stone-100">
            <p class="text-stone-500 mb-4">Suchen Sie nach mehr?</p>
            <NuxtLink 
              to="/furniture/all"
      ...
      >
        ...
        >
          <div class="bg-white p-8 max-w-md w-full shadow-xl">
            <h3 class="font-serif text-xl text-stone-900 mb-3">Wunschliste leeren?</h3>
            <p class="text-stone-600 mb-6">
              Sind Sie sicher, dass Sie alle {{ likeCount }} Artikel von Ihrer Wunschliste entfernen möchten? Diese Aktion kann nicht rückgängig gemacht werden.
            </p>
            <div class="flex gap-3">
              <button 
                @click="showClearModal = false"
                :disabled="isClearing"
                class="flex-1 px-6 py-3 border border-stone-200 text-stone-700 hover:bg-stone-50 transition-colors disabled:opacity-50"
              >
                Abbrechen
              </button>
              <button 
                ...
                :disabled="isClearing"
                class="flex-1 px-6 py-3 bg-rose-500 text-white hover:bg-rose-600 transition-colors disabled:opacity-50"
              >
                <span v-if="isClearing">Wird gelöscht...</span>
                <span v-else>Alle entfernen</span>
              </button>
...
const { sortedLikes, likeCount, removeLike, clearAllLikes, isSyncing } = useLikeList()
...
const handleRemove = async (productKey: string) => {
  removingIds.value.push(productKey)
  try {
    await removeLike(productKey)
  } finally {
    removingIds.value = removingIds.value.filter(id => id !== productKey)
  }
}
